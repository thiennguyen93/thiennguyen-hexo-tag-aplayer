...
var APlayerListTag = function (_BaseTag) {
  ...
  _createClass(APlayerListTag, [{
    ...
    value: function generate() {
      ...
      return `
<div id="${this.id}-placeholder" style="height: 90px" class="is-hidden"></div>
...
<script>
  ...
  if (!(window.aplayers.findIndex(item => item?.container?.getAttribute("id") === \`${this.id}\`) > -1)) {
    ...
    // Set link to page
    if (document.querySelector("#${this.id}")?.parentNode?.classList?.contains("thiennguyen-custom-content") && document.querySelector("#${this.id}").parentNode.dataset.pageUrl) {
      document.querySelector("#${this.id} .aplayer-title").innerHTML =\`<a class="link-muted" href="\${document.querySelector("#${this.id}").parentNode.dataset.pageUrl}">\${document.querySelector("#${this.id}").parentNode.dataset.pageTitle}</a>\`
    }
  }
  window.addEventListener("scroll", function() {
    ...
    if ( !window.thiennguyenPlayers?.["${this.id}"].paused && window.scrollY > (window.thiennguyenPlayers?.["${this.id}"].offsetTop + window.thiennguyenPlayers?.["${this.id}"].offsetHeight*2)) {
        ...
        document.getElementById("${this.id}-placeholder").classList.remove("is-hidden")
    } else {
        ...
        // Hide placeholder when media panel is in its correct position
        document.getElementById("${this.id}-placeholder").classList.add("is-hidden")
    }
  });
